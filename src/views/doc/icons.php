<blockquote class="layui-elem-quote">
    Yii2-theme-layui当前共引入<span class="layui-red iconsLength"></span>个外部图标。<span class="layui-word-aux">【点击可复制】此页面并非后台模版需要的，只是为了让大家了解都引入了哪些外部图标，实际应用中可删除。</span>
</blockquote>
<textarea id="z1copyText"></textarea>
<ul class="z1icons layui-row"></ul>
...
$iconUrl = $bundle->baseUrl . '/resources/css/font_seraph.css';
$js = <<<JS
layui.use(['form','layer','jquery'],function(){
    ...
    $.get('http://at.alicdn.com/t/font_900958_5oyw0iijsg6.css',function(data){
        var iconHtml = '';
        for(var i=1;i<data.split(".z1icon-").length;i++){
            iconHtml += "<li class='layui-col-xs4 layui-col-sm3 layui-col-md2 layui-col-lg1'>"+
                            "<i class='z1iconfont z1icon-" + data.split(".z1icon-")[i].split(":before")[0] + "'></i>" +
                            "z1icon-" + data.split('.z1icon-')[i].split(':before')[0] +
                        "</li>";
        }
        $(".z1icons").html(iconHtml);
        $(".iconsLength").text(data.split(".z1icon-").length-1);
    })

    $("body").on("click",".z1icons li",function(){
        var copyText = document.getElementById("z1copyText");
        copyText.innerText = $(this).text();
        copyText.select();
        document.execCommand("copy");
        layer.msg("复制成功",{anim: 2});
